...
pid_t           ei::self_pid;
sigset_t        ei::sigchld_mask;   // Signal mask for SIGCHLD synchronization
...
int main(int argc, char* argv[])
{
  ...
  // Make signal pipe larger to reduce chance of buffer overflow
  #ifdef F_SETPIPE_SZ
  int pipe_size = SIGCHLD_MAX_SIZE * 4; // 4x buffer size for safety
  if (fcntl(sigchld_pipe[1], F_SETPIPE_SZ, pipe_size) == -1) {
    DEBUG(debug, "Warning: Could not increase signal pipe buffer size: %s", strerror(errno));
  }
  #endif
  ...
  // Initialize signal mask for SIGCHLD synchronization to prevent races
  ...
  while (!terminated) {
    ...
    // Fix potential integer overflow in timeout calculations
    int secs = int(wakeup);
    double usec_frac = (wakeup - secs) * 1000000.0 + 0.5;
    // Clamp to prevent overflow
    if (usec_frac > 999999.0) usec_frac = 999999.0;
    if (usec_frac < 0.0) usec_frac = 0.0;
    ei::TimeVal timeout(secs, int(usec_frac));
