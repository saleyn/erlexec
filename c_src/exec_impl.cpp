...
//------------------------------------------------------------------------------
// Safe file descriptor management functions to prevent double-close vulnerabilities
//------------------------------------------------------------------------------
...
{
  // Got SIGCHLD event - process all available signals to prevent buffer overflow
  pid_t child;
  int   n;
  int   processed = 0;
  const int max_signals = static_cast<int>(SIGCHLD_MAX_SIZE / sizeof(pid_t));

  while ((n = read_sigchld(child)) > 0) {
    check_child_exit(child);
    processed++;
    // Prevent infinite loop in case of corrupted pipe data
    if (processed > max_signals) {
      DEBUG(debug, "Warning: processed %d SIGCHLD signals in one batch, breaking to prevent infinite loop", processed);
      break;
    }
  }
...
pid_t start_child(CmdOptions& op, std::string& error)
{
  ...
  if (pid < 0) {
    ...
    */
    ...
    // Apply per-command Linux capabilities before execve
    #ifdef HAVE_CAP
...
{
  // Use safe close to prevent double-close vulnerabilities
  for (int i=STDIN_FILENO; i<=STDERR_FILENO; i++) {
